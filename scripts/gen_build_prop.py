...
def get_build_keys(product_config):
  ...
  if default_cert == "" or default_cert == os.path.join(TEST_KEY_DIR, "testKey"):
    return "test-keys"
  if default_cert.startswith("vendor/lumine-priv/"):
    return "release-keys"
  return "dev-keys"
...
def parse_args():
  ...
  config["LumineDesc"] = config["BuildDesc"]
  config["LumineDevice"] = config["DeviceName"]
...
def generate_build_info(args):
  ...
  if config["BuildVariant"] != "eng":
    ...
    print(f"ro.build.display.id?={config['LumineDesc']}")
  ...
  print(f"org.luminedroid.device={config['LumineDevice']}")
